...
\begin{alltt}
\hlcom{#set.seed(1)}
\hlcom{#canvas_collatz(colors = colorPalette("tuscany3"))}
\hlcom{# help("aRtsy")}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\begin{alltt}
\hlcom{#set.seed(1313)}
\hlcom{#canvas_collatz(colors = colorPalette("vrolik4"), background = "#dbdbdb", n = 72, , angle.odd = -.05, angle.even = 0.0145)}
\end{alltt}
\end{kframe}
\end{knitrout}
%Code to insert a figure [H]ere
...
\begin{alltt}
\hlkwd{set.seed}(2)
\hlkwd{canvas_collatz}(colors = \hlkwd{colorPalette}(\hlsng{"azul"}), \hlkwd{background}(\hlsng{"fafafa"}), n = 100, angle.odd = .9 = angle.even = -.9)
\end{alltt}
\end{kframe}
\end{knitrout}
\begin{knitrout}
