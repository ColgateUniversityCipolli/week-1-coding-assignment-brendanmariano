...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#Sets seed and colorPalette to specified value}
...
\begin{alltt}
 \hlcom{#Changes seed}
 \hlcom{#Creates arrangement and shape of some strands}
...
\hlkwc{background} \hldef{=} \hlsng{"#dbdbdb"}\hldef{,} \hlkwc{n} \hldef{=} \hlnum{72}\hldef{, ,} \hlkwc{angle.odd} \hldef{=} \hlopt{-}\hlnum{.05}\hldef{,} \hlkwc{angle.even} \hldef{=} \hlnum{0.0145}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
%Code to insert a figure [H]ere
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#Self created plot}
\hlkwd{set.seed}\hldef{(}\hlnum{2}\hldef{)}
\hlkwd{canvas_collatz}\hldef{(}\hlkwc{colors} \hldef{=} \hlkwd{colorPalette}\hldef{(}\hlsng{"azul"}\hldef{),} \hlkwc{background} \hldef{=} \hlsng{"#fafafa"}\hldef{,}
\hlkwc{n} \hldef{=} \hlnum{100}\hldef{,} \hlkwc{angle.odd} \hldef{=} \hlnum{.9}\hldef{,} \hlkwc{angle.even} \hldef{=} \hlopt{-}\hlnum{.9}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
\begin{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{vector} \hlkwb{=} \hlkwd{seq}\hldef{(}\hlkwc{from} \hldef{=} \hlnum{0}\hldef{,} \hlkwc{to} \hldef{=} \hlnum{10}\hldef{,} \hlkwc{by} \hldef{=} \hlnum{1}\hldef{)}
\hldef{vector}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#Performs computations for each vector value}
\hldef{result} \hlkwb{=} \hlnum{2}\hlopt{^}\hldef{(vector} \hlopt{+} \hlnum{1}\hldef{)} \hlopt{+} \hlnum{2}\hlopt{^}\hldef{(vector}\hlopt{-}\hlnum{1}\hldef{)}
\hldef{result}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
  \hlcom{#Uses which function to find index that contains 40}
  \hlcom{#which produces 40 in the equation}
  \hldef{index} \hlkwb{=} \hlkwd{which}\hldef{(result} \hlopt{==} \hlnum{40.0}\hldef{)}
  \hldef{index}
\end{alltt}
\begin{verbatim}
## [1] 5
...
\textbf{Solution: This approach wouldn't work using the vector approach because x needs to be an index, which means that it has to be a whole number. }
\end{enumerate}
\end{enumerate}
